...
static void intertile_audiopipeline_RECV_thread(QueueHandle_t input_queue)
{
    int msg_length;
    int32_t *msg;
...
{
    xTaskCreate((TaskFunction_t) intertile_audiopipeline_RECV_thread,
                "intertile_ap_thread",
                RTOS_THREAD_STACK_SIZE(intertile_audiopipeline_RECV_thread),
                input_queue,
...
void tcp_to_pipeline_intertile_create( void ){
    ...
    if( input_queue != NULL )
    {
        tcp_to_queue_handle_t tcp_to_codec_handle = tcp_to_queue_create(
                input_queue,
        ...
        intertile_pipeline_client_init(
                tcp_to_codec_handle->queue);
        tcp_to_queue_stream_create(
                tcp_to_codec_handle,
                (appconfINTERTILE_AUDIOPIPELINE_RECV_TASK_PRIORITY + 1) );
    }
}
